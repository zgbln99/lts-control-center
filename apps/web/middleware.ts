...
function requiredApiRoles(pathname:string,method:string):Role[]|null{
  ...
  if(pathname.startsWith('/api/users')||pathname.startsWith('/api/settings')) return ['ADMIN'];
...
export async function middleware(request:NextRequest){
  ...
  try{
    ...
    const required=pathname.startsWith('/api/')?requiredApiRoles(pathname,request.method):requiredPageRoles(pathname);
    ...
    headers.set('x-lts-user-id',session.sub);
    headers.set('x-lts-user-role',session.role);
