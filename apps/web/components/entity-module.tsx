...
export function EntityModule({title,subtitle,endpoint,itemsKey,columns,fields=[],createLabel='Neu anlegen',emptyText='Noch keine Einträge.',allowDelete=true,deleteLabel='Archivieren'}:Props){
  ...
  function startCreate(){setSelected(null);setFormOpen(true);setMessage('')}
  function startEdit(row:any){setSelected(row);setFormOpen(true);setMessage('')}
  ...
  async function remove(row:any){
    if(!allowDelete||!window.confirm(`${deleteLabel}: ${columns.length?displayValue(valueAt(row,columns[0].key),'text'):'Eintrag'}?`)) return;
  ...
  return <>
    <div className="modulePageHead"><div><h2>{title}</h2><p>{subtitle}</p></div><div className="moduleHeadActions"><button className="filterBtn" onClick={()=>void load()}><RefreshCw size={14}/>Aktualisieren</button>{fields.length>0&&<button className="greenBtn" onClick={startCreate}><Plus size={15}/>{createLabel}</button>}</div></div>
