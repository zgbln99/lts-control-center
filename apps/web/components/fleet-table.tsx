...
function TuvBadge({v}:{v:Vehicle}){return <span className={'badge '+(v.tuvState==='ok'?'badgeOk':v.tuvState==='warning'?'badgeWarn':'badgeBad')}>{v.tuv}</span>}

export function FleetTable({onSelect}:{onSelect:(v:Vehicle)=>void}){
 const [query,setQuery]=useState('');
 const data=useMemo(()=>vehicles.filter(v=>[v.plate,v.vehicle,v.vin,v.inventory].join(' ').toLowerCase().includes(query.toLowerCase())),[query]);
