...
export default function Fuhrpark(){
 ...
 const [selected,setSelected]=useState<Vehicle|null>(fallbackVehicles[3]);
 const [usingDatabase,setUsingDatabase]=useState(false);
 const [reloadToken,setReloadToken]=useState(0);
 const [fleetView,setFleetView]=useState<'all'|'trailer'>('all');

 useEffect(()=>{
   const controller = new AbortController();
   const search=typeof window!=='undefined'?window.location.search:'';
   const params=new URLSearchParams(search);
   setFleetView(params.get('typ')==='anhaenger'?'trailer':'all');
   fetch(`/api/vehicles${search}`,{signal:controller.signal})
     ...
     .then((payload:{vehicles?:ApiVehicle[]})=>{
       ...
       setFleet(mapped);setUsingDatabase(true);
       setSelected(current=>mapped.find(vehicle=>vehicle.plate===current?.plate) ?? mapped[0] ?? null);
     })
     .catch(error=>{if(error?.name!=='AbortError') console.info('Using Fuhrpark demo fallback until database is available.')});
 ...
 const kpis=[
   {icon:Truck,value:String(stats.total),label:fleetView==='trailer'?'Anhänger / Auflieger':'Fahrzeuge gesamt',sub:usingDatabase?'aus Fahrzeugbestand':'+5 diese Woche',kind:'green'},
 ...
 const upcoming=useMemo(()=>fleet.flatMap(vehicle=>(vehicle.upcoming ?? []).map(item=>({...item,plate:vehicle.plate}))).sort((a,b)=>new Date(a.dueDate).getTime()-new Date(b.dueDate).getTime()).slice(0,4),[fleet]);
 const fallbackTerms=[{plate:'TF-LS 1152',type:'TÜV',dueDate:'2026-09-24T00:00:00.000Z'},{plate:'TF-LS 999',type:'SP',dueDate:'2026-09-27T00:00:00.000Z'},{plate:'TF-LS 1116',type:'Tacho',dueDate:'2026-10-01T00:00:00.000Z'},{plate:'TF-LS 1131',type:'TÜV',dueDate:'2026-10-02T00:00:00.000Z'}];
 ...
 return <div className="appShell"><Sidebar/><main className="main"><Topbar title={fleetView==='trailer'?'Anhänger':'Fuhrpark'} subtitle={fleetView==='trailer'?'Anhänger und Auflieger':'Übersicht aller Fahrzeuge'}/><div className="content">
   ...
   <div className="mainGrid"><div><FleetTable vehicles={fleet} onSelect={setSelected}/>{selected?<VehicleDetail vehicle={selected} onChanged={()=>setReloadToken(value=>value+1)}/>:<div className="tableCard emptyFleetCard">Keine Fahrzeuge in dieser Ansicht.</div>}</div><aside className="rightRail">
