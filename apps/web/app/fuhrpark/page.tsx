...
export default function Fuhrpark(){
 ...
 return <div className="appShell"><Sidebar/><main className="main"><Topbar title={fleetView==='trailer'?'Anhänger':'Fuhrpark'} subtitle={fleetView==='trailer'?'Anhänger und Auflieger':'Übersicht aller Fahrzeuge'}/><div className="content">
   ...
   <div className="mainGrid"><div><FleetTable vehicles={fleet} onSelect={setSelected} onChanged={reload}/>{fleet.length===0&&<div className="tableCard emptyFleetCard">Keine Fahrzeuge in dieser Ansicht.</div>}{selected&&<VehicleDetail vehicle={selected} onClose={()=>setSelected(null)} onChanged={reload}/>}</div><aside className="rightRail">
