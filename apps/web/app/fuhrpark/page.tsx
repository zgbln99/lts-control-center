...
export default function Fuhrpark(){
 const [fleet,setFleet]=useState<LoadedVehicle[]>([]); const [selected,setSelected]=useState<Vehicle|null>(null); const [loadError,setLoadError]=useState(''); const [reloadToken,setReloadToken]=useState(0); const [fleetView,setFleetView]=useState<'all'|'trailer'>('all'); const [routeSearch,setRouteSearch]=useState('');
 useEffect(()=>{const sync=()=>setRouteSearch(window.location.search);sync();const click=()=>window.setTimeout(sync,0);window.addEventListener('popstate',sync);document.addEventListener('click',click);return()=>{window.removeEventListener('popstate',sync);document.removeEventListener('click',click)}},[]);
 useEffect(()=>{const controller = new AbortController();const search=routeSearch;const params=new URLSearchParams(search);setFleetView(params.get('typ')==='anhaenger'?'trailer':'all');setLoadError('');fetch(`/api/vehicles${search}`,{signal:controller.signal}).then(response=>{if(!response.ok) throw new Error(`Fleet API ${response.status}`);return response.json()}).then((payload:{vehicles?:ApiVehicle[]})=>{const mapped=Array.isArray(payload.vehicles)?payload.vehicles.map(mapApiVehicle):[];setFleet(mapped);setSelected(current=>current?mapped.find(vehicle=>vehicle.plate===current.plate)??null:null)}).catch(error=>{if(error?.name!=='AbortError'){setFleet([]);setSelected(null);setLoadError('Fahrzeugdaten konnten nicht geladen werden.')}});return ()=>controller.abort()},[reloadToken,routeSearch]);
 ...
 return <div className="appShell"><Sidebar/><main className="main"><Topbar title={fleetView==='trailer'?'Anhänger':'Fuhrpark'} subtitle={fleetView==='trailer'?'Anhänger und Auflieger':'Übersicht aller Fahrzeuge'}/><div className="content">
   ...
   <div className="mainGrid"><div><FleetTable vehicles={fleet} selected={selected} onSelect={setSelected} onClose={()=>setSelected(null)} onChanged={reload}/>{fleet.length===0&&<div className="tableCard emptyFleetCard">Keine Fahrzeuge in dieser Ansicht.</div>}</div><aside className="rightRail">
