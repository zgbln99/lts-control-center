...
type ApiVehicle = {
  id: string; plate: string; category:string; vehicle: string; firstRegistration: string | null; vin: string | null;
...
type LoadedVehicle = Vehicle & { samsaraOnline?: boolean | null; category?:string; upcoming?: { type: 'TÜV' | 'SP' | 'Tacho'; dueDate: string; state: DeadlineState }[] };
...
export default function Fuhrpark(){
 ...
 useEffect(()=>{
   ...
   const params=new URLSearchParams(search); setFleetView(params.get('typ')==='anhaenger'?'trailer':'all'); setLoadError('');
   fetch(`/api/vehicles${search}`,{signal:controller.signal})
     ...
     .then((payload:{vehicles?:ApiVehicle[]})=>{const mapped=Array.isArray(payload.vehicles)?payload.vehicles.map(mapApiVehicle):[];setFleet(mapped);setSelected(current=>mapped.find(vehicle=>vehicle.plate===current?.plate) ?? mapped[0] ?? null)})
     .catch(error=>{if(error?.name!=='AbortError'){setFleet([]);setSelected(null);setLoadError('Fahrzeugdaten konnten nicht geladen werden.')}});
 ...
 const kpis=[
   {icon:Truck,value:String(stats.total),label:fleetView==='trailer'?'Anhänger / Auflieger':'Fahrzeuge gesamt',sub:'aus Fahrzeugbestand',kind:'green'},
 ...
 const terms=useMemo(()=>fleet.flatMap(vehicle=>(vehicle.upcoming ?? []).map(item=>({...item,plate:vehicle.plate}))).sort((a,b)=>new Date(a.dueDate).getTime()-new Date(b.dueDate).getTime()).slice(0,4),[fleet]);
