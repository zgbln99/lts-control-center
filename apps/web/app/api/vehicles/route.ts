...
export async function GET(request:NextRequest) {
  ...
  const categoryWhere=typ==='anhaenger'?{in:['TRAILER','SEMITRAILER'] as any[]}:category?category as any:undefined;
  const vehicles=await prisma.vehicle.findMany({where:{lifecycle:'ACTIVE',...(categoryWhere?{category:categoryWhere}: {})},orderBy:[{sourcePosition:'asc'},{plate:'asc'}],include:{telemetry:true,deadlines:{where:{completedAt:null},orderBy:{dueDate:'asc'}},_count:{select:{documents:true}}}});
