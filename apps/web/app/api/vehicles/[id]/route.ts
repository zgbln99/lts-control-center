...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}) {
  ...
  if('category' in body){const parsed=category(body.category);if(!parsed)return NextResponse.json({error:'Invalid vehicle category'},{status:400});data.category=parsed}
  ...
  const decimalFields=['taxMonthAmount','taxQuarterAmount','taxSumAmount','monthlyRate','powerKw','powerHp'];for(const field of decimalFields){if(!(field in body))continue;const parsed=optionalDecimal(body[field]);if(parsed===undefined)return NextResponse.json({error:`Invalid ${field}`},{status:400});data[field]=parsed}
