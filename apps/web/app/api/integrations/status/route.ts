...
export async function GET(){
  const [vehicleCount,samsaraVehicles,telemetryCount,latestTelemetry,documentCount,latestDocument,driverCount,activeDriverCount,folderMappings,states,dddBatches,dddViolations,latestDdd]=await Promise.all([
    ...
    prisma.driver.count({where:{samsaraId:{not:null}}}),
    prisma.driver.count({where:{samsaraId:{not:null},status:'ACTIVE'}}),
  ...
  return NextResponse.json({generatedAt:new Date().toISOString(),integrations:{
    samsara:{configured:Boolean(process.env.SAMSARA_API_TOKEN),connectedVehicles:samsaraVehicles,activeVehicles:vehicleCount,telemetryRecords:telemetryCount,drivers:driverCount,activeDrivers:activeDriverCount,lastSyncAt:state.SAMSARA?.lastSyncAt?.toISOString()??latestTelemetry?.syncedAt?.toISOString()??null,lastStatus:state.SAMSARA?.lastStatus??null},
