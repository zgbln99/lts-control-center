...
function normalizeSinglePlate(raw, inheritedPrefix = null) {
  ...
  if (/^\d+[A-Z]?$/.test(value) && inheritedPrefix) value = `${inheritedPrefix} ${value}`;
...
function parsePlateExpression(rawValue) {
  ...
  return { original, plate: aliases[0] ?? original.toUpperCase(), aliases };
}
...
function parseLifecycle(row) {
  ...
  if (lower.includes('verkauft')) {
    ...
    return { lifecycle: 'SOLD', lifecycleNote: note, soldAt: germanTextDateToIso(note), soldTo: cleanString(buyerMatch?.[1]) };
  }
  if (lower.includes('abgemeldet')) return { lifecycle: 'ARCHIVED', lifecycleNote: note, soldAt: null, soldTo: null };
...
const workbook = await readExcelFile(input);
const fleetSheet = workbook.find(sheet => sheet.sheet === 'Tabelle1');
if (!fleetSheet) throw new Error('Missing worksheet: Tabelle1');
const fleetRows = fleetSheet.data;
...
const vehicles = rawFleet.map(row => {
  ...
  const financingEndIso = dateToIso(row['Vertragsende, wenn finanziert']);
  const registeredAtIso = dateToIso(row['angemeldet am']);
  const firstRegistrationIso = dateToIso(row['Erstzulassung']);
  const numericRate = asDecimal(row['Rate']);
  const lifecycle = parseLifecycle(row);
  return {
    sourcePosition: asInteger(row['Pos.']), plate: plate.plate, plateOriginal: plate.original, plateAliases: plate.aliases,
    firstRegistration: firstRegistrationIso, vin: cleanString(row['Kfz-Ident. Nummer']), insuranceNumber: asIdentifier(row['Versicherungsnummer']),
    taxMonthAmount: asDecimal(row['Monat']), taxQuarterAmount: asDecimal(row['Quartal']), taxNumber: cleanString(row['Kfz St.nummer']),
    taxSumAmount: asDecimal(row['Summe Steuer']), grossVehicleWeightKg: asInteger(row['Gesamtmasse']), documentsNotes: cleanString(row['Unterl. vorhanden']),
    financingEnd: financingEndIso, financingEndRaw: financingEndIso ? null : cleanString(row['Vertragsende, wenn finanziert']), registeredAt: registeredAtIso,
    monthlyRate: numericRate, rateRaw: numericRate === null ? cleanString(row['Rate']) : null, inventoryNumber: asIdentifier(row['Inventarnummer']),
    notes: cleanString(row['Besonderheiten']), lifecycle: lifecycle.lifecycle, lifecycleNote: lifecycle.lifecycleNote, soldAt: lifecycle.soldAt, soldTo: lifecycle.soldTo, sourceRaw: row,
  };
});

const hookLoadSheet = workbook.find(sheet => sheet.sheet === 'Hakenlast');
let hookLoadPeriods = [];
if (hookLoadSheet) {
  const rows = hookLoadSheet.data;
  ...
  hookLoadPeriods = objects.map(row => ({
    plateText: cleanString(row['Kennzeichen']), normalizedPlate: normalizeSinglePlate(row['Kennzeichen']),
    startsAt: dateToIso(row['ab wann']), endsAt: dateToIso(row['bis wann']), sourceRaw: row,
  })).filter(row => row.plateText && row.startsAt);
}

const duplicateCanonicalPlates = Object.entries(vehicles.reduce((acc, vehicle) => {
