...
async function setState(status,message,counters){await prisma.integrationState.upsert({where:{key:'SAMSARA'},create:{key:'SAMSARA',enabled:true,lastSyncAt:new Date(),lastStatus:status,lastMessage:message,counters},update:{enabled:true,lastSyncAt:new Date(),lastStatus:status,lastMessage:message,counters}})}
...
async function main() {
  ...
  const byVin=new Map(dbVehicles.filter(v=>v.vin).map(v=>[v.vin.toUpperCase(),v]));const byPlate=new Map();for(const vehicle of dbVehicles){byPlate.set(vehicle.plate.toUpperCase(),vehicle);for(const alias of vehicle.plateAliases)byPlate.set(alias.toUpperCase(),vehicle)}
  const samsaraVehicles=await paginate('/fleet/vehicles',{limit:512});const stats=await paginate('/fleet/vehicles/stats',{types:'gps,obdOdometerMeters,gpsOdometerMeters'});const statsById=new Map(stats.map(item=>[String(item.id),item]));
  let matched=0,unmatched=0,telemetryUpdated=0;const matchedDbIds=new Set();const unmatchedSamsara=[];
  for(const samsaraVehicle of samsaraVehicles){const vin=samsaraVin(samsaraVehicle);const plate=samsaraPlate(samsaraVehicle);const existing=(vin?byVin.get(vin.toUpperCase()):null)||(plate?byPlate.get(plate.toUpperCase()):null);if(!existing){unmatched+=1;unmatchedSamsara.push({id:String(samsaraVehicle.id),name:samsaraVehicle.name??null,vin,plate});continue}matched+=1;matchedDbIds.add(existing.id);const names=displayName(samsaraVehicle,existing);await prisma.vehicle.update({where:{id:existing.id},data:{samsaraId:String(samsaraVehicle.id),manufacturer:names.manufacturer,model:names.model,displayName:names.displayName}});const stat=statsById.get(String(samsaraVehicle.id));if(!stat)continue;const gps=stat.gps??null;const geofenceName=clean(gps?.address?.name)||null;const reverseGeo=clean(gps?.reverseGeo?.formattedLocation)||null;const locationLabel=geofenceName||reverseGeo||null;await prisma.vehicleTelemetry.upsert({where:{vehicleId:existing.id},create:{vehicleId:existing.id,samsaraId:String(samsaraVehicle.id),online:onlineFromGps(gps),odometerKm:odometerKm(stat),latitude:typeof gps?.latitude==='number'?gps.latitude:null,longitude:typeof gps?.longitude==='number'?gps.longitude:null,address:reverseGeo,locationLabel,geofenceName,lastSeenAt:gps?.time?new Date(gps.time):null,raw:stat},update:{samsaraId:String(samsaraVehicle.id),online:onlineFromGps(gps),odometerKm:odometerKm(stat),latitude:typeof gps?.latitude==='number'?gps.latitude:null,longitude:typeof gps?.longitude==='number'?gps.longitude:null,address:reverseGeo,locationLabel,geofenceName,lastSeenAt:gps?.time?new Date(gps.time):null,raw:stat}});telemetryUpdated+=1}
  ...
  const counters={samsaraVehicles:samsaraVehicles.length,samsaraStats:stats.length,matched,unmatched,telemetryUpdated,databaseWithoutMatch:dbWithoutMatch.length};
  await setState('OK',`Matched ${matched} of ${samsaraVehicles.length} Samsara vehicles.`,counters);
  console.log(JSON.stringify({...counters,baseUrl,unmatchedSamsara:unmatchedSamsara.slice(0,100),databaseVehiclesWithoutSamsaraMatch:dbWithoutMatch.slice(0,100)},null,2));
}

main().catch(async error=>{console.error(error);try{await setState('ERROR',String(error?.message??error).slice(0,1000),{})}catch(stateError){console.error('Could not persist Samsara error state',stateError)}process.exitCode=1}).finally(async()=>{await prisma.$disconnect()});
